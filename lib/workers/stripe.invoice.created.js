    ...
    .then(function fetchOrganization (stripeEvent) {
      ...
      return bigPoppa.getOrganizations({ stripeCustomerId: stripeCustomerId })
        ...
        .tap(function payInvoice (org) {
          const invoiceIsClosed = stripeEvent.data.object.close
          const invoiceHasBeenPaid = stripeEvent.data.object.paid
          const invoiceId = stripeEvent.data.object.id
          const orgDoesNotCurrentlyHaveActiveSubscription = org.isInGracePeriod || !org.allowed
          log.trace(
            { invoiceHasBeenPaid, invoiceId, invoiceIsClosed, orgDoesNotCurrentlyHaveActiveSubscription },
          ...
          if (
            ...
            const id = org.id
            log.trace({ id, invoiceId }, 'Enqueuing new job to create subscription')
