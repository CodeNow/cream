...
server.start()
  .then(() => {
    ...
    jobIntervals = [
      setInterval(
        rabbitmq.publishCheckForAlmostExpiredOrganizations.bind(rabbitmq),
        process.env.TRIAL_ALMOST_EXPIRED_CHECK_INTERVAL
      ),
      setInterval(
        rabbitmq.publishCheckForExpiredOrganizations.bind(rabbitmq),
        process.env.TRIAL_EXPIRATION_CHECK_INTERVAL
      ),
      setInterval(
        rabbitmq.publishCheckForAlmostInactiveOrganizations.bind(rabbitmq),
        process.env.ALMOST_INACTIVE_CHECK_INTERVAL
      ),
      setInterval(
        rabbitmq.publishCheckForOrganizationPaymentHaveFailed.bind(rabbitmq),
