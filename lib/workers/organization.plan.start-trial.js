...
module.exports = function CreateOrganizationInStripeAndStartTrial (rawJob) {
  const log = logger.child({ rawJob: rawJob, method: 'CreateOrganizationInStripeAndStartTrial' })
    ...
    .then(function fetchOrgnization (job) {
      ...
      return bigPoppa.getOrganization(job.organizationId)
        .then(function createCustomerInStripe (res) {
          ...
          return stripe.createCustomerAndSubscriptionForOrganization(org, job.planId || null)
        })
