...
module.exports.task = function CheckForOrganizationsWithEndingTrials (job) {
  ...
  return OrganizationService.getFilteredOrgsInTrialByTrialEndTime(threeDaysFromNow)
  ...
  .then(function updateNotifiedTrialEndingInStripe (orgsWithEndingTrials) {
    ...
    return Promise.filter(orgsWithEndingTrials, org => {
      ...
      return stripe.subscriptions.updateSubscriptionWithTrialEndingNotification(org.subscription.id, now.toISOString())
      .catch(err => {
