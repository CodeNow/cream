...
const eventSchema = Joi.object({
  ...
  type: Joi.string().valid('invoice.payment_failure').required(), // Don't allow any other type of event
  data: Joi.object({
    object: Joi.object({
...
module.exports = function ProcessPaymentFailure (job) {
  ...
  return stripe.getEvent(job.stripeEventId)
    ...
    .then(function getInvoice (stripeEvent) {
      ...
      return stripe.getInvoice(invoiceId)
      .then(function checkIfNotificationHasBeenSent (invoice) {
        ...
        return Promise.all([
          bigPoppa.getOrganizations({ stripeCustomerId }),
          stripe.getCustomerPaymentMethodOwner(stripeCustomerId)
        ])
      })
      .then(function saveUpdatesToOrganization ([orgs, paymentMethodOwner]) {
        let org = orgs[0]
        if (!org) {
        ...
        return stripe.updateInvoiceWithAdminPaymentFailedNotification(invoiceId)
        .then(() => {
          rabbitmq.publishEvent('organization.invoice.payment-failure', {
