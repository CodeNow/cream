...
module.exports.task = function ProcessPaymentSucceeded (job) {
  ...
  return stripe.getEvent(job.stripeEventId)
    ...
    .then(function fetchOrganization (stripeEvent) {
      ...
      let subscriptionLineItem = stripeEvent.data.object.lines.data.find(item => item.type === 'subscription')
      if (!subscriptionLineItem) {
        throw new WorkerStopError('There were no subscription line items found for this invoice')
      }
      // Stripe API always uses UNIX timestamps
      let newActivePeriodEnd = moment(subscriptionLineItem.period.end, 'X')
      let stripeCustomerId = stripeEvent.data.object.customer
      log.trace({ newActivePeriodEnd: newActivePeriodEnd.toISOString(), stripeCustomerId }, 'Getting organization')
