...
module.exports.task = function ProcessPaymentSucceeded (job) {
  ...
  return stripe.getEvent(job.stripeEventId)
    ...
    .then(function fetchOrganization (stripeEvent) {
      ...
      let stripeSubscriptionId = stripeEvent.data.object.subscription
      log.trace({ stripeCustomerId, stripeSubscriptionId }, 'Fetch organizations and subscription')
      return Promise.join(
        bigPoppa.getOrganizations({ stripeCustomerId }),
        stripe.subscriptions.get(stripeSubscriptionId)
      )
