...
module.exports = function CheckInvoicedPaymentFailed (job) {
  ...
  .filter(function filterAlreadyPaid (organizationWith48HoursLeftInGracePeriod) {
    return !keypather.get(organizationWith48HoursLeftInGracePeriod, 'invoice.paid')
  })
  ...
  .filter(function updateSubscriptionProperty (org) {
    return stripe.invoices.updateNotifiedAllMembersPaymentFailed(org.invoice.id, moment().toISOString())
