...
module.exports = function CheckInvoicedPaymentFailed (job) {
  ...
  // Get latests invoice
  .filter(function getCurrentInvoiceForOrganization (org) {
    return stripe.invoices.getCurrentInvoiceForOrganization(org)
  ...
  .filter(function updateSubscriptionProperty (org) {
    ...
    let userId = org.paymentMethodOwner.id
    let time = moment().toISOString()
    return stripe.invoices.updateNotifiedAllMembersPaymentFailed(invoiceId, userId, time)
      .return(true)
