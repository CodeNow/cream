...
module.exports = class StripeInvoiceUtils {
   ...
   * @returns {Promise}
   */
  static getCurrentInvoiceForOrganization (org) {
    const log = logger.child({ org })
    log.info('StripeInvoiceUtils.getLastInvoiceForOrganization called')
   ...
   * @returns {Promise}
   */
  static updateNotifiedAllMembersPaymentFailed (invoiceId, userId, notificationSentTime) {
    const log = logger.child({ invoiceId, userId, notificationSentTime }, 'Stripe.updateNotifiedAllMembersPaymentFailed')
