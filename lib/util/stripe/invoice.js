...
module.exports = class StripeInvoiceUtils {
   ...
   * @returns {Promise}
   */
  static getCurrentInvoice (org) {
    const orgStripeCustomerId = org.stripeCustomerId
    const orgStripeSubscriptionId = org.stripeSubscriptionId
    const log = logger.child({ orgStripeCustomerId, orgStripeSubscriptionId })
    ...
    return stripeClient.invoices.list({
      ...
      .then(res => {
        ...
        invoices = invoices
          .filter(invoice => invoice.subscription === orgStripeSubscriptionId)
  ...
  /**
   * Pay an invoice. This takes away the ability to make any modifications to invoice.
   *
   * @param {String}     invoiceId - Stripe invoice ID
   * @resolves {Object}  invoice   - Stripe Invoice object
   * @returns {Promise}
   */
  static pay (invoiceId) {
    const log = logger.child({ invoiceId, method: 'StripeInvoiceUtils.pay' })
    log.info('called')
    return stripeClient.invoices.pay(invoiceId)
