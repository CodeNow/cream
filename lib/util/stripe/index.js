 ...
 /**
  ...
  static getCustomerPaymentMethodOwner (orgStripeCustomerId) {
    ...
    return Stripe.getCustomer(orgStripeCustomerId)
      .then(function (stripeCustomer) {
        log.trace('Customer retrieved')
        const paymentMethodOwnerId = keypather.get(stripeCustomer, 'metadata.paymentMethodOwnerId')
        const paymentMethodOwnerGithubId = keypather.get(stripeCustomer, 'metadata.paymentMethodOwnerGithubId')
        if (!paymentMethodOwnerId || !paymentMethodOwnerGithubId) {
          throw new EntityNotFoundError('No paymentMethodOwner found for this org', { orgStripeCustomerId })
        }
...
module.exports.invoice = StipreInvoiceUtils
