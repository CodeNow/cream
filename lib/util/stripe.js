...
module.exports = class Stripe {
  ...
  static updateSubsriptionWithTrialExipredNotification (stripeSubscriptionId, notificationSentTime) {
    const log = logger.child({ stripeSubscriptionId: stripeSubscriptionId, notificationSentTime: notificationSentTime }, 'Stripe.updateCustomerTrialExpired')
    log.info('Stripe.updateCustomerTrialExpired called')
    const updates = {
      notifiedTrialExpired: notificationSentTime
    }
    return stripeClient.customers.update(stripeSubscriptionId, updates)
  }
