...
module.exports = class Stripe {
   ...
   *
   * @param {Object}     org               - Organization object
   * @param {String}     org.stripeCustomerId - Organization id
   * @resolves {Object}  res               - Response object
   * @return {Promise}
   */
  static getCustomer (org) {
    const log = logger.child({ orgStripeCustomerId: org.stripeCustomerId })
    log.info('getCustomer called')
    return Promise.try(() => {
      if (!org.stripeCustomerId) {
        log.warn({ org: org }, 'No `stripeCustomerId` provided')
        throw new EntityNotFoundError('Customer has no `stripeCustomerId`')
      }
      return stripeClient.customers.retrieve(org.stripeCustomerId)
    })
