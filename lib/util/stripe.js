...
const planNames = Object.keys(PLANS).map(k => PLANS[k])

const updatePlanForCustomerSchema = Joi.object({
  stripeCustomerId: Joi.string().required().regex(/^cus_/),
  plan: Joi.string().valid(planNames),
  planUsers: Joi.array().itmes(Joi.string())
})
...
module.exports = class Stripe {
    ...
    return Promise.resolve()
      ...
      .then(function fetchPlanMetadata (stripeCustomer) {
        ...
        return Promise.props({
          plan: Stripe.getPlanForOrganization(org.githubId),
  ...
  static updateOrganization (org, planId) {
    const log = logger.child({ method: 'Stripe.updateCustomer', org: org, planId: planId })
    log.info('Stripe.updateCustomer called')
    return Promise.resolve()
      .then(function fetchPlanMetadata (stripeCustomer) {
        log.trace({ stripeCustomer: stripeCustomer }, 'fetchPlanMetadata')
        return Promise.props({
          stripeCustomer: stripeCustomer
          subscription: Stripe.getSubscriptionForOrganization(org.stripeCustomerId)
        })
      })
      .then(function updateCustomerInStripe (res) {
        log.trace({ planId: res.planId, users: org.users }, 'updateCustomerInStripe')
        let planUsers = Stripe.getPlanUsersForOrganization(org.users || [])
        log.trace({ users: planUsers }, 'Determine plan users')
        return stripeClient.subscriptions.create({
          customer: res.stripeCustomer.id,
  ...
  static updatePlanForOrganization (opts) {
    return Joi.validateAsync(opts, updatePlanForCustomerSchema)
      .then(function updatePlan () {
        let updates = {
  ...
  static getSubscriptionForOrganization (orgStripeCustomerId) {
    ...
    return stripe.subscriptions.list({ limit: 1, customer: orgStripeCustomerId })
      .then(res => {
