...
module.exports = class Stripe {
  ...
  static updatePlanIdForOrganizationBaseOnCurrentUsage (org) {
    const log = logger.child({ org: org })
    log.info('Stripe.updatePlanIdForOrganizationBaseOnCurrentUsage called')
    return Promise.props({
      subscription: Stripe._getSubscriptionForOrganization(org.stripeCustomerId),
      planId: Stripe.getPlanIdForOrganizationBasedOnCurrentUsage(org.githubId)
    })
      .then(function updateCustomerInStripe (subscription, planId) {
        log.trace({ subscription: subscription, planId: planId }, 'fetched customer and planId')
        return Stripe._updatePlanInSubscription(subscription.id, planId)
      })
  }
  ...
  static updatePaymentMethodForOrganization (org, stripeToken, user) {
    ...
    return Promise.try(() => {
      ...
      return stripeClient.customers.update(org.stripeCustomerId, {
        source: stripeToken,
        metadata: {
          paymentMethodOwnerId: user.id,
          paymentMethodOwnerGithubId: user.id
        }
      })
    })
  }
  ...
  static _updatePlanInSubscription (subscriptionId, planId) {
    const log = logger.child({ subscriptionId: subscriptionId, planId: planId }, 'Stripe.updatePlanInSubscription')
    log.info('updatePlanInSubscription called')
    return stripeClient.subscriptions.update(
      subscriptionId,
      { plan: planId }
    )
