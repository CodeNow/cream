  ...
  static getPlan (validatedReq, res) {
    ...
    .then(function getCurrentPlans (org) {
      return Promise.all([
        ...
        .spread(function fetchStripePlans (currentSubscription, planIdForNextPlan) {
          return Promise.all([
        ...
        .spread(function (currentPlan, nextPlan, currentSubscription) {
          let currentNumberOfUsers = null
          if (currentSubscription.metadata.users) {
            let users = JSON.parse(currentSubscription.metadata.users)
            currentNumberOfUsers = users.users.length
          }
          return {
            current: {
              plan: Object.assign({}, currentPlan, { userCount: currentNumberOfUsers })
            },
            next: {
              plan: Object.assign({}, nextPlan, { userCount: org.users.length })
            }
          }
        })
    })
  ...
  static getPaymentMethod (validatedReq, res) {
    ...
    return bigPoppa.getOrganization(validatedReq.params.id)
      ...
      .then(function formatResponse (customer) {
        ...
        return {
          owner: {
            id: keypather.get(customer, 'metadata.paymentMethodOwnerId'),
            githubId: keypather.get(customer, 'metadata.paymentMethodOwnerGithubId')
          },
