        ...
        return response.data.map(invoice => {
          ...
          invoice = util.convertObjectToCamelCase(Object.assign({}, invoice, {
            paidBy: {
              ...
              id: parseInt(keypather.get(invoice, 'metadata.paymentMethodOwnerId'), 10),
              githubId: parseInt(keypather.get(invoice, 'metadata.paymentMethodOwnerGithubId'), 10)
            }
          }))
      ...
      .then(function formatResponse (customer) {
        ...
        return {
          owner: {
            ...
            id: parseInt(keypather.get(customer, 'metadata.paymentMethodOwnerId'), 10),
            githubId: parseInt(keypather.get(customer, 'metadata.paymentMethodOwnerGithubId'), 10)
          },
