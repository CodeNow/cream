...
class OrganizationRouter extends BaseRouter {
        ...
        return response.data.map(invoice => {
          ...
          invoice = util.convertObjectToCamelCase(Object.assign({}, invoice, {
            paidBy: {
              ...
              id: +(keypather.get(invoice, 'metadata.paymentMethodOwnerId')),
              githubId: +(keypather.get(invoice, 'metadata.paymentMethodOwnerGithubId'))
            }
          }))
          ...
          delete invoice.webhooksDeliveredAt
  ...
  static getPlan (validatedReq, res) {
    ...
    return bigPoppa.getOrganization(validatedReq.params.id)
      ...
      .spread(function (currentPlan, nextPlan, currentSubscription, org) {
